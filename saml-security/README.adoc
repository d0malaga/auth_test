...
= saml-security: reuses app-profile-saml-jee-jsp for saml authentication with a role mapper
:author: Tomas Aronsson
:level: Unknown
:technologies: JSP, WildFly Elytron, Keycloak, SAML

[abstract]
Demonstrate how to configure Wildlfy Elytron with a keycloak Adapter for saml reusing the login/role UI code from the keycloak Quickstart app-profile-saml-jee-jsp. Adds role info to UI and role mappers.
...
* https://www.keycloak.org/docs/latest/securing_apps/#_saml-general-config
 ...
 * Configuration for the built-in Keycloak role mapper
 * A basic example for a custom role mappings provider
...
[source,subs="+quotes,attributes+",options="nowrap"]
----
env THIS_HOST=192.168.1.155 mvn k8s:resource
----
 ...
 * K8s: http://<THIS_HOST>:31380/{artifactId}-wildfly/
 ** users: appadmin/abcd123, appuser/abcd123, guest/abcd123
 * The IdP runs on the port 8081/31381, user: admin, password admin
...
When you access the application you are redirected to a Keycloak login form. Once you have logged into the application you see the app-profile-saml-jee-jsp app welcome page.
