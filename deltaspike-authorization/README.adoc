...
:level: Beginner
:technologies: JSF, CDI, Deltaspike, Docker, jkube

[abstract]
Demonstrate the creation of a custom authorization example using @SecurityBindingType from DeltaSpike. This is an updated version of the Wildfly quickstart which uses the jkube maven plugin to create a docker file and reduces manual steps to get started without installing a local Wildfly.
...
To restrict who can invoke a method, we create an annotation, called a security binding type. This quickstart has two security binding types - `@AdminAllowed` and `@AppuserAllowed`.
...
When you click on the `Appuser Method` button you will see the following message: `You executed a @AppuserAllowed method` - you are authorized to invoke this method.
