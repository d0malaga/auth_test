...
1. copied from wf quickstart - deltaspike-authorization, adds jkube (docker+k8s)
2. ldap-security using an openldap docker container instead of wf accounts
3. openid-security using a keycloak 10.0.0 IdP container + service adapter
4. Angular 10 UI on top of WF kitchensink quickstart
5. todo: saml-security using a keycloak 10.0.0 IdP container + service adapter
...
| link:saml-security/README{outfilesuffix}[saml-security]|JSF, CDI, Deltaspike, WildFly Elytron, Keycloak, OpenID | Demonstrate how to configure Wildlfy Elytron with a keycloak Adapter for openid reusing the login/role UI code from Quickstart deltaspike-authorization.  | Unknown | _none_
|===
//</TOC>
